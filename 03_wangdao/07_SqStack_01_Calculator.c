...
int calculator(char *expression, int *res);
...
int calculator(char *expression, int *res)
{
    ...
    while (*p)
    {
        ...
        else if (*p >= '0' && *p <= '9')
        {
            ...
            if (!push(&numStack, num))
            {
                // 栈满退出
                printf("01calculator fail,case by :!push(numStack, *(p++)) is 0\n");
                return 0;
            }
            num = 0;
        }
        // 3. 遇到 + - 运算符，如果符号栈栈顶元素不为空或不为 '('
        // ，依次弹出符号栈中元素与数字栈中的2元素做运算后再入数字栈直到 '(' 结束，在把当前符号入栈
        // ，否则直接入符号栈。
        else if (*p == '+' || *p == '-')
        {
            if (isEmpty(opStack) || getTop(opStack) == '(')
            {
                if (!push(&opStack, *p))
                {
                    printf("02calculator fail,case by :!push(&opStack, *(p++)) is 0\n");
                    return 0;
                }
            }
            else
            {
                ...
                {
                    if (!pop(&opStack, &popElem))
                    {
                        printf("03calculator fail,case by : !pop(&opStack) is 0\n");
                        return 0;
                    }
                    calBetweenNum(popElem, &numStack);
                }
                if (getTop(opStack) == '(')
                {
                    if (!pop(&opStack, &popElem))
                    {
                        printf("03.1calculator fail,case by : !pop(&opStack) is 0\n");
                        return 0;
                    }
                }
            }
        ...
        // 4. 遇到 * /运算符，如果符号栈顶元素不为空且不为'('且栈顶元素大于等于当前符号优先级，则依次弹出符号栈元素并弹出
        // 数字栈中的两个元素计算后再加入到数字栈，直到遇到'('或栈为空结束，否则直接把当前符号加入到符号栈中。
        else if (*p == '*' || *p == '/')
        {
            if (isEmpty(opStack) || getTop(opStack) == '(')
            {
                if (!push(&opStack, *p))
                {
                    printf("04calculator fail,case by :!push(&opStack,*(p++)) is 0\n");
                    return 0;
                }
            }
            else
            {
                while (!isEmpty(opStack) && getTop(opStack) != '(' && compareOperator(getTop(opStack), *p) >= 0)
                {
                    if (!pop(&opStack, &popElem))
                    {
                        printf("05calculator fail,case by : !pop(&opStack) is 0\n");
                        return 0;
                    }
                    calBetweenNum(popElem, &numStack);
                }
                if (getTop(opStack) == '(')
                {
                    if (!pop(&opStack, &popElem))
                    {
                        printf("05.1calculator fail,case by : !pop(&opStack) is 0\n");
                        return 0;
                    }
                }
            }
        ...
        else if (*p == '(')
        {
            if (!push(&opStack, *(p++)))
            {
                printf("06calculator fail,case by :!push(&opStack,*(p++)) is 0\n");
                return 0;
            }
        }
        // 6. 遇到 ')' ，依次弹出1个符号栈中元素 和 2个数字栈元素进行计算，把计算结果加入数字栈中，直到遇到'('结束
        else if (*p == ')')
        {
            while (getTop(opStack) != '(')
            {
                if (!pop(&opStack, &popElem))
                {
                    printf("07calculator fail,case by : !pop(&opStack) is 0\n");
                    return 0;
                }
                calBetweenNum(popElem, &numStack);
            }
            ...
            if (!pop(&opStack, &popElem))
            {
                printf("07.1calculator fail,case by : !pop(&opStack) is 0\n");
                return 0;
            }
            p++;
        }
        ...
        {
            printf("08calculator fail,case by : The charater is illeagl\n");
            return 0;
        }
    }
    // 7. 中缀读取完毕，弹出符号栈中所有符号计算得出结果
    while (getTop(opStack))
    {
        if (!pop(&opStack, &popElem))
        {
            printf("09calculator fail,case by : !pop(&opStack) is 0\n");
            return 0;
        }
        calBetweenNum(popElem, &numStack);
    }
...
int main()
{
    ...
    // char *express01 = "2*3";
    char *express02 = "2*(10/(3+2))";
    // char *express02 = "10/(3+2)";

    result = -1;
    // calculator(express02, &result);
